...
price_fig = px.box(
    data[(data["price"] < 4500) & (data["free"] == 0)],
    x="genre",
    y="price",
...
review_agg = (
    data.groupby(["genre", "analysis_split"]).agg({"reviews": "median"}).reset_index()
)
review_fig = go.Figure(
    data=[
        go.Bar(
            name="Apps with Rating < 3.7",
            x=review_agg[
                (review_agg["analysis_split"] == 0) & (review_agg["reviews"] < 5000)
            ]["genre"],
            y=review_agg[
                (review_agg["analysis_split"] == 0) & (review_agg["reviews"] < 5000)
            ]["reviews"],
        ...
        go.Bar(
            name="Apps with Rating >=3.7",
            x=review_agg[
                (review_agg["analysis_split"] == 1) & (review_agg["reviews"] < 5000)
            ]["genre"],
            y=review_agg[
                (review_agg["analysis_split"] == 1) & (review_agg["reviews"] < 5000)
            ]["reviews"],
...
year_agg = (
    data.groupby(["genre", "analysis_split"])
    .agg({"Years_from_release": "median"})
    .reset_index()
)
...
col_list = {
    "Size vs Genre": style_fig,
    "Price vs Genre": price_fig,
    "Reviews vs Genre": review_fig,
    "Years from Release vs Genre": year_fig,
    "In App Product Price vs Genre": IAPprice_fig,
}

desc_list = {
    "Size vs Genre": """This is a simple Bar Chart which shows the Median size (in MB) of all apps in a genre,
...
insight_list = {
    "Size vs Genre": """Have added this graph here mainly for people to get an idea of how big an app they could keep
    ...
    [
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H2("Welcome to the App Rating Predictor Website"),
                        html.Br(),
                        html.Br(),
                        html.Br(),
                        html.P(
                            "Use the menu on the top right corner to navigate through the page"
                        ),
                        html.Br(),
                        html.P(
                            "You can view a few graphs in the Visualisation tab along with some of my insights from that graph"
                        ),
                        html.Br(),
                        html.P(
                            "Or you can test out the model to predict the rating your app would get!!"
                        ),
                        html.Br(),
                    ],
                ),
...
layout_page_1 = html.Div(
    children=[
        ...
        html.P(
            "You'll be shown the graph along with a slight description of what the graph is about as well as a few of my insights from it"
        ),
        html.Div(
            [
                html.Br(),
            ...
            style={"color": "#7fDBFF", "backgroundColor": "#111111"},
        ),
        html.Br(),
    ...
    style={"textAlign": "center", "color": "#7fDBFF", "backgroundColor": "#111111"},
)

layout_page_2 = html.Div(
    children=[
        html.H4(
            children="Enter the following details about your app and click on the 'Get Rating' button",
            style={
                "textAlign": "center",
                "color": "#7fDBFF",
                "backgroundColor": "#111111",
        ...
        dcc.Input(
            id="android",
            type="text",
            placeholder="Required Android Version",
            style={"marginLeft": "180px", "width": "15%", "textAlign": "center"},
        ),
        dcc.Input(
            id="ads",
            type="text",
            placeholder="App Contains Ads? (Y/N)",
            style={"marginLeft": "50px", "width": "15%", "textAlign": "center"},
        ),
        dcc.Input(
            id="currency",
            type="text",
            placeholder="The currency you deal in?",
            style={"marginLeft": "50px", "width": "15%", "textAlign": "center"},
        ),
        html.Br(),
        html.Br(),
        dcc.Input(
            id="devel",
            type="text",
            placeholder="Developing Company",
            style={"marginLeft": "100px", "width": "15%", "textAlign": "center"},
        ),
        dcc.Input(
            id="genre",
            type="text",
            placeholder="Genre of the App",
            style={"marginLeft": "50px", "width": "15%", "textAlign": "center"},
        ),
        dcc.Input(
            id="offersIAP",
            type="text",
            placeholder="Do you have In-App Purchases?(Y/N)",
            style={"marginLeft": "50px", "width": "15%", "textAlign": "center"},
        ),
        dcc.Input(
            id="price",
            type="number",
            placeholder="Price of the App",
            min=0,
            style={"marginLeft": "50px", "width": "15%", "textAlign": "center"},
        ),
        html.Br(),
        ...
        dcc.Input(
            id="size",
            type="text",
            placeholder="Estimated Size of the App(in MB)",
            style={"marginLeft": "100px", "width": "17.5%", "textAlign": "center"},
        ),
        dcc.Input(
            id="product_price",
            type="text",
            placeholder="Average Price of the In-App-Purchase Product",
            style={"marginLeft": "50px", "width": "25%", "textAlign": "center"},
        ),
        dcc.Input(
            id="min_age",
            type="text",
            placeholder="Minimum Age Required for Usage(in Years)",
            style={"marginLeft": "15px", "width": "20%", "textAlign": "center"},
        ),
        html.Br(),
        html.Br(),
        dcc.Input(
            id="years_from_release",
            min=0,
            type="number",
            placeholder="After how many years from the App's Release do you want it's rating?",
            style={"marginLeft": "350px", "width": "45%", "textAlign": "center"},
        ),
        html.Br(),
        html.Br(),
        html.Button(
            "Get Rating", id="get_rating", n_clicks=0, style={"marginLeft": "725px"}
        ),
        html.Br(),
        html.Br(),
        html.P(
...
@app.callback(
    ...
    Input(component_id="date_text", component_property="date"),
...
def get_rating(
    ...
    if get_rating > 0:
        try:
            currency = les["currency"].transform([currency.upper()])
        except:
            currency = -1
        ...
        try:
            android = int(android.split(" ")[1].split(".")[0])
        except:
            android = -1
        ...
        pred = np.argmax(
            np.sum(
                models["model_0"].predict_proba(data)
