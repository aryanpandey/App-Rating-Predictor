...
layout_page_2 = html.Div(
    ...
    dcc.Input(id="offersIAP", type="text", placeholder="Do you have In-App Purchases?(Y/N)",
              style = {'marginLeft':'50px', 'width':'15%', 'textAlign':'center'}),
    dcc.Input(id="price", type="number", placeholder="Price of the App", min = 0,
              style = {'marginLeft':'50px', 'width':'15%', 'textAlign':'center'}),
    html.Br(),
    html.Br(),    
    ...
    html.Br(),
    html.Br(),
    dcc.Input(id="years_from_release", min = 0, type="number", placeholder="After how many years from the App's Release do you want it's rating?",
              style = {'marginLeft':'350px', 'width':'45%', 'textAlign':'center'}),
    html.Br(),
    html.Br(),    
    html.Button("Get Rating", id = 'get_rating', n_clicks=0, style={'marginLeft':'725px'}),
    html.Br(),
    html.Br(),
...
def get_rating(date_text, android, ads, currency, devel, genre, offersIAP, price,
    ...
    if get_rating>0:
        ...
        pred = np.argmax(np.sum(models['model_0'].predict_proba(data) + 
                          models['model_1'].predict_proba(data) +
                          models['model_2'].predict_proba(data) + 
                          models['model_3'].predict_proba(data) +
                          models['model_4'].predict_proba(data), axis=0))

        print(np.sum(models['model_0'].predict_proba(data) + 
                          models['model_1'].predict_proba(data) +
                          models['model_2'].predict_proba(data) + 
                          models['model_3'].predict_proba(data) +
                          models['model_4'].predict_proba(data), axis=0))
        score_dict = {0:'0-2',1:'2-3.5',2:'3.5-3.75',3:'3.75-4', 4:'4-4.2', 5:'4.2-4.4',6:'4.4-4.6', 7:'4.6-4.75', 8:'4.75-4.5'}
